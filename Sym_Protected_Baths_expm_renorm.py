#Works better than Sym_Protected_Baths_Even_Odd_Projections because redefines the zero point of the energies to keep the matrix exponentiations convergent
...
#Bath Properties (note the cutoffs are chosen to match Delta and Lambda within the Delta/Lambda Loop)
...
for d in range(num_tested_deltas):
    ...
    for lam in lam_list:
        ...
        even_slices = np.concatenate((np.arange(0, vib, 2), np.arange(vib + 1, 2*vib, 2)))
        odd_slices = np.concatenate((np.arange(1, vib, 2), np.arange(vib, 2*vib, 2)))

        H_even = Ham[even_slices][:, even_slices]
        H_odd = Ham[odd_slices][:, odd_slices]


        e_evals, e_evecs = scipy.linalg.eigh(H_even)
        o_evals, o_evecs = scipy.linalg.eigh(H_odd)

        H_even = H_even - min(e_evals)*np.eye(len(even_slices))
        H_odd = H_odd - min(o_evals)*np.eye(len(odd_slices))

        rho_e = scipy.linalg.expm(-1*beta_e*H_even)
        rho_e = pe*rho_e/np.trace(rho_e)
        rho_o = scipy.linalg.expm(-1*beta_o*H_odd)
        rho_o = po*rho_o/np.trace(rho_o)

        rho_full = np.empty((2*vib, 2*vib))

        #see https://stackoverflow.com/questions/53652185/modifying-values-of-a-sub-array-in-python and https://docs.scipy.org/doc/numpy/reference/arrays.indexing.html
        rho_full[np.ix_(even_slices, even_slices)] = rho_e
        rho_full[np.ix_(odd_slices, odd_slices)] = rho_o
        rho_full[np.ix_(odd_slices, even_slices)] = np.zeros((len(odd_slices), len(even_slices)))
        rho_full[np.ix_(even_slices, odd_slices)] = np.zeros((len(even_slices), len(odd_slices)))




        rho_ss_site = rho_full
...
file_name = os.path.join("Rabi_Data", f"{datetime.today().strftime('%Y-%m-%d')}_vib={vib}_dellam_scan_Rabi_symbaths")
extend = 0
while os.path.exists(file_name + ".npy"):
    extend += 1
    file_name = os.path.join("Rabi_Data", f"{datetime.today().strftime('%Y-%m-%d')}_vib={vib}_dellam_scan_Rabi_symbaths_{extend}")
...
if tested_delta_only:
    ...
    single_del_file_name = os.path.join("Single_Del_Cross_Sections", f"{datetime.today().strftime('%Y-%m-%d')}_vib={vib}_dellam_scan_Rabi_symbaths_{extend}")
