#Use this for the symmetry broken steady state, but not when symmetry is not broken
...
#beta_o = 0.4
beta_o = 0.4
even_coupling = 1e-5 #Even and odd coupling for symmetry-protected bath (but I doubt these matter since the even and odd spaces should separately thermalize)
odd_coupling = 1e-5
prop_time = 1e12 #Want to propagate long enough so that we get an NESS
#del_lam_scan
    ...
    for lam in lam_list:
        ...
        if sec_mode == "sec":
            ...
            if sym_breaking:
                ...
                rho_t = Full_Redfield.full_sec_prop(pop_derivative, coh_derivatives, [prop_time], pops_0, coh_0)[0]
        ...
        p3_ppt.negativity_all_subdivisions(rho_t_site, [2, vib], 1e-8, return_negativity_sum=True)[1]
        stored_ee_ss_rho[*index_tuple] = rho_t
        stored_site_ss_rho[*index_tuple] = rho_t_site
